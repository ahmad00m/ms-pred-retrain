...
# from src.ms_pred.magma import fragmentation
...
def get_output_dict(
    ...
    # Merge duplicates by formula (keep max intensity)
    formula_idx_dict = {}
    uniq_mask = []
    for idx in range(len(formulas)):
        formula = formulas[idx]
    ...
    if spec_intens.size == 0:
        ...
        output_tbl = {
            "mz": list(spec_masses),
            "ms2_inten": list(spec_intens),
            "rel_inten": list(spec_intens),  # already normalized upstream
            "mono_mass": list(formula_masses),
            "formula_mass_no_adduct": list(formula_mass_no_adduct),
            "mass_diff": list(min_mass_diff),
            "formula": list(formulas),
...
# --- NEW: make the worker function picklable (no lambda) ---
def export_wrapper(arg):
    return get_output_dict(**arg)
...
def main():
    ...
    for _, row in labels_df.iterrows():
        spec = row["spec"]
        new_entry = {
            "spec": input_specs_dict.get(spec),  # slight hardening vs KeyError
            "form": row["formula"],
            "mass_diff_type": mass_diff_type,
            "spec_name": spec,
            "mass_diff_thresh": mass_diff_thresh,
...
if __name__ == "__main__":
    # Use a safer start method for RDKit/HDF5 + multiprocessing
    try:
